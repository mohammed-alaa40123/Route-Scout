...
def resolve_dataset_dir(framework, traffic_mode, split, topo, model_repo):
    """
    Resolve dataset directory for TrafficModels or Scheduling.
    """

    split = split.lower()
    topo = topo.lower()

    if model_repo == "Scheduling":
        if framework == "erlang":
            root = os.path.join(ERLANG_DIR, "data", "scheduling", traffic_mode)
        else:
            root = os.path.join(FERMI_DIR, "data", "scheduling", traffic_mode)

        return os.path.join(root, split, topo)

    # ---------------- TrafficModels (original behavior) ----------------
    if framework == "erlang":
    ...
    if split == "test" and topo != "gbn":
        raise SystemExit("For test split, topology must be gbn.")
    if split == "train" and topo not in ("geant2", "nsfnet"):
        raise SystemExit("For train split, topology must be geant2 or nsfnet.")

    if traffic_mode == "all_multiplexed":
        topo_dir = f"{topo}-multiplexed"
    elif traffic_mode == "constant_bitrate":
        topo_dir = f"{topo}-constant"
    else:
        topo_dir = f"{topo}-{traffic_mode}"

    return os.path.join(root, split, topo_dir)
...
def erlang_import_modules(model_repo):
    """
    Add RouteNet-Erlang/<model_repo> to sys.path and import Erlang modules.
    model_repo: "TrafficModels" or "Scheduling"
    """
    tm_dir = os.path.join(ERLANG_DIR, model_repo)
    if tm_dir not in sys.path:
        sys.path.insert(0, tm_dir)

    from datanetAPI import DatanetAPI

    # Try to import network_to_hypergraph; fallback to sample_to_dependency_graph if unavailable.
    try:
        from read_dataset import network_to_hypergraph
    except Exception:
        try:
            from read_dataset import sample_to_dependency_graph as network_to_hypergraph
        except Exception:
            raise ImportError(f"Cannot import network_to_hypergraph or sample_to_dependency_graph from {tm_dir}")
...
def run_erlang(args):
    ...
    # Import Erlang modules using selected repository (TrafficModels / Scheduling)
    DatanetAPI, network_to_hypergraph, GNN_Model = erlang_import_modules(args.model_repo)
    ...
    # Use selected model_repo for experiment dir
    exp_dir = os.path.join(ERLANG_DIR, args.model_repo, metric_cap, args.traffic_mode)
...
def fermi_import_modules(metric, model_repo):
    """
    Add RouteNet-Fermi/<model_repo> to sys.path and import metric-specific
    model + data_generator.
    model_repo: "TrafficModels" or "Scheduling"
    For Fermi, if model_repo == "TrafficModels" we map to folder "traffic_models"
    (lowercase) which is the original name in the repo. If model_repo == "Scheduling",
    we use "Scheduling" as given.
    """
    # Map to actual folder name expected by Fermi repo
    if model_repo == "TrafficModels":
        tm_dir_name = "traffic_models"
    else:
        tm_dir_name = model_repo  # e.g., "Scheduling"

    tm_dir = os.path.join(FERMI_DIR, tm_dir_name)
    if tm_dir not in sys.path:
        sys.path.insert(0, tm_dir)
    ...
    return DatanetAPI, FermiModel, network_to_hypergraph, hypergraph_to_input_data, subdir, tm_dir_name
...
def run_fermi(args):
    ...
    DatanetAPI, FermiModel, network_to_hypergraph, hypergraph_to_input_data, subdir, model_repo_dir = \
        fermi_import_modules(metric, args.model_repo)
    ...
    sample, x_base, y_base, path_src_dst_base, path_routes_base = fermi_get_sample_features(
        dataset_dir, args.sample_index, metric, DatanetAPI, network_to_hypergraph, hypergraph_to_input_data
    )
    ...
    for cand in candidates:
        ...
        # Validate candidate path against this topology
        validate_candidate_path_for_sample(
            sample, cand_id, path_nodes, framework_tag="Fermi", args=args
        )
        ...
        x_cand, path_src_dst_cand, path_routes_cand = fermi_build_features_with_override(
            sample, override, network_to_hypergraph, hypergraph_to_input_data
        )
...
def main():
    ...
    parser.add_argument(
        "--model_repo",
        type=str,
        default="TrafficModels",
        choices=["TrafficModels", "Scheduling"],
        help="Which model repository to read checkpoints from (TrafficModels or Scheduling).",
    )
    parser.add_argument(
        "--metric",
        type=str,
        default="delay",
        choices=["delay", "jitter", "losses"],
        help="Target metric.",
    )
    parser.add_argument(
        "--traffic_mode",
    ...
    parser.add_argument(
        ...
        type=str,
        default="gbn",
        choices=["gbn", "geant2", "nsfnet","rediris"],
